...
async function getDataFromToken(encodedToken) {
    try {
        const payload = await jwt.verify(encodedToken, config.jwt_secret);
        if (payload != null) {
...
app.post("/reports/updateReport", async (req, res) => {
    ...
    if (req.cookies != null && "reports_auth" in req.cookies) {
        user = await getDataFromToken(req.cookies.reports_auth);
    }
    if (user == null) {
...
app.post('/reports/createReport', async (req, res) => {
    if (req.cookies != null && "reports_auth" in req.cookies) {
        if (req.body != null && "text" in req.body) {
            ...
            const user = await getDataFromToken(encodedToken);
            if (user != null) {
